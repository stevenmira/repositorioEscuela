...
class HojaVidaController extends Controller
{
    ...
    */
    public function index(Request $request)
    {
        ...
        if($request)
        {
            ...
            ->select('ma.nombre','ma.apellido','ma.fotografia','ma.direccion','ma.nip', 'ma.sexo', 'ma.mdui', 'ma.fechanacimiento', 'ho.cargo','ho.id_hoja')
            ->groupBy('ma.nombre','ma.apellido','ma.fotografia','ma.direccion','ma.nip', 'ma.sexo', 'ma.mdui', 'ma.fechanacimiento', 'ho.cargo','ho.id_hoja')
            ->where('ma.nombre','LIKE','%'.$query.'%')
            ->orwhere('ma.apellido','LIKE','%'.$query.'%')
            ->orwhere('ma.apellido','LIKE','%'.$query.'%')
            ->orwhere('ma.sexo','LIKE','%'.$query.'%')
            ->orwhere('ma.nip','LIKE','%'.$query.'%')
            ->orderBy('ma.id','desc')
            ->paginate(9);
    ...
    */
     ...
     public function store (HojaVidaFormRequest $request)
    {
        ...
        try{
                ...
                $extendido = $request->get('extendido');
                if ($extendido !="") {
    ...
    */
    ...
    {   
        ...
        {
          DB::rollback();
          Session::flash('unicidad', ''.' No se pudo actualizar, el DUI ingresado ya existe');
          return Redirect::to('docente/cvitae');
        }
