...
class AsignacionController extends Controller
{
    ...
    public function store(AsignacionFormRequest $request)
    {
        ...
        //if ($tm->tipom==1) {
            ...
            $asignacion=Asignacion::where('mdui',$maestroR)->where('anioasignacion',$request->get('idanio'))->first();
                ...
                if(!is_null($asignacion))
                {
                    ...
                    if($resultado->ciclo==2 or $resultado->ciclo==3)
                    {
                       if($ciclo==2 or $ciclo==3){
                        ...
                        $resul=DetalleAsignacion::where('iddetallegrado',$iddg)->where('aniodetalleasignacion',$request->get('idanio'))->first();
                        if(is_null($resul)){
